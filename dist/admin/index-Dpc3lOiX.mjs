...
const index = {
  register(app) {
    app.addMenuLink({
      ...
      Component: () => import("./Chat-DOly8mRB.mjs").then((mod) => ({ default: mod.Chat }))
    });
    app.addSettingsLink(
      {
        id: PLUGIN_ID,
      ...
      },
      {
        ...
        Component: () => import("./Settings-DmHeJAux.mjs").then((mod) => ({ default: mod.Settings }))
      }
    );
    app.registerPlugin({
      id: PLUGIN_ID,
  ...
  async registerTrads({ locales }) {
    return Promise.all(
      locales.map(async (locale) => {
        try {
          const { default: data } = await __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => import("./en-uzX6hxgv.mjs") }), `./translations/${locale}.json`, 3);
