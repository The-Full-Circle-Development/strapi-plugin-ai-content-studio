...
const index = {
  register(app) {
    app.addMenuLink({
      ...
      Component: () => import("./Chat-CRA4ID_2.mjs").then((mod) => ({ default: mod.Chat }))
    });
    app.addSettingsLink(
      {
        id: PLUGIN_ID,
      ...
      },
      {
        ...
        Component: () => import("./Settings-nmxPNHOA.mjs").then((mod) => ({ default: mod.Settings }))
      }
    );
    app.registerPlugin({
      id: PLUGIN_ID,
  ...
  async registerTrads({ locales }) {
    return Promise.all(
      locales.map(async (locale) => {
        try {
          const { default: data } = await __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => import("./en-Cz2Dsx0E.mjs") }), `./translations/${locale}.json`, 3);
