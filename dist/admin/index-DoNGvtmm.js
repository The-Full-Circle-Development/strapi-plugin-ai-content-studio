...
const index = {
  register(app) {
    app.addMenuLink({
      ...
      Component: () => Promise.resolve().then(() => require("./Chat-CyM7g1r_.js")).then((mod) => ({ default: mod.Chat }))
    });
    app.addSettingsLink(
      {
        id: PLUGIN_ID,
      ...
      },
      {
        ...
        Component: () => Promise.resolve().then(() => require("./Settings-CHVe2CD0.js")).then((mod) => ({ default: mod.Settings }))
      }
    );
    app.registerPlugin({
      id: PLUGIN_ID,
  ...
  async registerTrads({ locales }) {
    return Promise.all(
      locales.map(async (locale) => {
        try {
          const { default: data } = await __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => Promise.resolve().then(() => require("./en-BJ_RyDQU.js")) }), `./translations/${locale}.json`, 3);
