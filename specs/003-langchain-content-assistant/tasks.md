...
>
> **Every implementation task is complete**; `corepack pnpm@10 run typecheck`,
> `corepack pnpm@10 run test` (4 suites, 133 tests) and `corepack pnpm@10 run build` are all clean,
> and `dist/` is rebuilt.
>
> **Nothing has been committed.** The constitution's per-commit gate has an item this environment
> cannot satisfy — item 4, manual verification in a running Strapi admin panel — and there is no host
> app here. Committing against an unmet gate item is what the gate exists to prevent, so the nine
> commits in *Commit mapping* below are left for the maintainer to make after the quickstart passes.
>
> Tasks marked **NOT DONE** are exactly the verification tasks that need that panel.
>
> Three design corrections were made against the installed packages, and the affected documents were
> updated in place: the bridge is pinned to `@ai-sdk/langchain@2.0.285` (not 3.0.93), `createAgent`
> takes `model`/`systemPrompt` (not `llm`/`prompt`), and the bridge's `onAbort` cannot suppress its
> error chunk. See *Deviations from the plan* at the end of this file.
...
- [X] T001 Amend `.specify/memory/constitution.md` 1.0.0 → 1.1.0 in its **own `docs:` commit, before every other commit in this feature**, with four changes and no others:
...
- [X] T002 Add the LangChain tree to `devDependencies` in `package.json` at the versions verified in research D8 — `langchain@1.5.10`, `@langchain/core@1.2.9`, `@langchain/anthropic@1.5.9`, `@langchain/openai@1.5.11`, `@langchain/google-genai@2.3.0`, `@ai-sdk/langchain@3.0.93` — never `peerDependencies` (Principle IV), then `corepack pnpm@10 install`. **Then answer research D17's three findings against the real install, before any code is written against the bridge**: (a) does esbuild bundle the ESM-only `@ai-sdk/langchain` cleanly into the CommonJS server bundle; (b) does its `engines: node >= 22` break an install on Node 20, which this repo and Strapi v5 both support — if it does, moving the floor is a documented breaking change for consumers, not a silent bump; (c) does its hard `ai@7.0.93` dependency dedupe against this repo's `ai@6.0.208` or arrive as a **second copy** — run `corepack pnpm@10 why ai` and read the lockfile. A second major of `ai` in one bundle means two incompatible `UIMessageChunk` identities and a possible move in the stored `parts` shape, which is the one thing FR-013 cannot absorb. If it is a second copy, decide before T015: upgrade this repo to `ai@7` and re-run the FR-013 shape diff for real, or pin an older bridge whose peer is `ai@6`
- [X] T003 Remove `@ai-sdk/anthropic`, `@ai-sdk/openai` and `@ai-sdk/google` from `devDependencies` in `package.json`; **keep** `ai` and `@ai-sdk/react` — they are the wire and storage contract, not providers (contracts/removals.md §3, §4)
- [X] T004 [P] Confirm the bundle baseline before any source change by running `wc -c dist/server/index.js` and `du -sh dist/`, and correct `specs/003-langchain-content-assistant/quickstart.md` → Setup if it differs from the recorded 1,600,257 bytes / 4.5 MB
- [X] T005 [P] Confirm no LangSmith tracing is enabled by this plugin: grep `server/src/` and `dist/` for `LANGSMITH_` and `LANGCHAIN_` and record the result against quickstart A12 (Principle I — a tracing client would ship prompts off the host)
- [X] T005a Stand up the repository's first test runner: **jest**, chosen because it is what Strapi itself uses — verified this session in the installed packages (`@strapi/strapi` → `"test:unit": "jest"`, `@strapi/sdk-plugin` → `"test:unit": "node --experimental-vm-modules node_modules/jest/bin/jest.js"`). Add it plus a TypeScript transform to `devDependencies`, and a `test` script to `package.json`. The transform is picked and **verified against the installed toolchain in the session that adds it**, not recalled — this project is TypeScript + CommonJS, and the wrong transform is a config rabbit hole, not a compile error. **Standing rule for every suite below: no test may call a language model, open a network connection, bootstrap the Strapi runtime, or touch the filesystem outside its own fixtures.** Every suite covers a pure function whose determinism is already a stated requirement, so a failure is a real defect and never a flake
...
- [X] T006 Extend the shared type surface in `server/src/types.ts`: add `ProviderDescriptor` (data-model §1), widen `StudioSettings.activeProvider` from its literal union to `string`, widen `providers` to `Record<string, ProviderState>`, add `ProviderState.baseUrl: string | null`, add `StudioSettings.grounding: { enabled: boolean }`, add `MaskedProviderState.baseUrl`, add `InstructionSet`, `InstallDescription` and `PublishOutcome`, and extend `ChangeItemOutcome` with `publish?: PublishOutcome` — no `any` in any exported signature (audit-type **removals** belong to US2/T040)
- [X] T007 Rewrite the normalization in `server/src/services/config.ts`: an extensible `providers` map that **preserves unknown keys** on read and write, `baseUrl` validation built on **`z.url({ protocol: /^https?$/ })`** from the zod v4 this repo already validates every request body with — verified in this session to accept `https://api.example.com/v1` and `http://h:8080` and reject `/v1`, `ftp://x.com` and `http:example.com`. **Do not reach for `z.httpUrl()`**: it pins zod's `domain` regex, which was verified to reject `http://localhost:11434/v1`, `http://127.0.0.1:8080/v1` and `http://ollama:11434/v1` — precisely the self-hosted `openai-compatible` endpoints this feature exists to serve. The userinfo rejection and the trailing-slash trim stay ours (`z.url()` accepts `https://user:pw@host`), as a `.refine()` and a `.transform()` reusing the existing idiom at `config.ts:187`. `grounding` defaulting to `{ enabled: true }`, `isSet` always recomputed from `apiKeyEnc`, a missing field always taking its default so an upgrade never breaks an existing install, and `getDecryptedKey` still decrypting **only** the requested provider (data-model §3). Also add **`isGroundingEnabled()`**, the single place the two-switch precedence of contracts/install-description.md §7 is computed — `pluginConfig.grounding.enabled && settings.grounding.enabled`. Every caller reads it; two flags with the rule re-derived per call site is how they end up disagreeing
...
- [X] T010 [P] [US1] Create the declarative provider table in `server/src/services/providers.ts`: **static** imports of `ChatAnthropic` (`@langchain/anthropic`), `ChatOpenAI` (`@langchain/openai`) and `ChatGoogleGenerativeAI` (`@langchain/google-genai`), and four `ProviderDescriptor` rows — `anthropic`, `openai`, `google`, `openai-compatible` — using the verified constructor options (`apiKey` + `model` everywhere; `configuration.baseURL` for `ChatOpenAI`, `baseUrl` for Google), `requiresBaseUrl` true **only** for `openai-compatible`, `useResponsesApi` left at its verified `false`, and a per-descriptor `supportsVision` implementing the **four declared rules** of contracts/provider-layer.md §3 — ported **verbatim** from today's `modelSupportsVision()` in `server/src/services/registry.ts` (`anthropic`: `claude-` prefix; `google`: `gemini-` prefix; `openai`: `gpt-4`/`gpt-5`/`o<digit>` minus the `gpt-3.5` and non-text exclusions; `openai-compatible`: never), default-deny for anything a rule cannot answer. **Leaving the three first-party descriptors at a bare default-deny is a silent regression** — image input works on all three today, it fails no negative test, and only quickstart A13 catches it. No dynamic `import()`, no `initChatModel`, no `switch` on provider identity (contracts/provider-layer.md §1, §3)
- [X] T011 [P] [US1] Create the client-side provider catalog in `admin/src/data/providers.ts` — id, English label, `requiresBaseUrl`, and `hasCuratedModels` derived from `MODELS[id] != null`. A **separate module**: nothing may be appended to `admin/src/data/models.ts` or restructure its `MODELS` literal (research D15)
- [X] T012 [US1] Rewrite `server/src/services/registry.ts`: **delete** the `switch (activeProvider)`, resolve the descriptor from the table on **every** request from persisted config, raise `NO_ACTIVE_PROVIDER` / `PROVIDER_DISABLED` / `MISSING_KEY` / `MISSING_BASE_URL` (new) / `UNKNOWN_PROVIDER` **before generation begins** with messages that name the provider and never the key, return `{ model, provider, modelId, supportsVision }` where `model` is a `BaseChatModel` instance, and replace the prefix-based `modelSupportsVision()` with the descriptor's declared rule (contracts/provider-layer.md §2)
- [X] T013 [US1] Update `server/src/controllers/settings.ts`: `GET` returns the masked shape including `grounding` and `baseUrl` **in full** (it is configuration, not a secret); `PUT` zod-validates `activeProvider` against the table (`400 unknown_provider` naming it), stores `activeModel` **verbatim** with no validation against any curated list and no normalization, keeps `apiKey` write-only (absent keeps the stored key, explicit `null` clears it, never echoed), validates `baseUrl` with a `400` naming the field, rejects an unknown provider key on write, and responds with the same masked shape so the client rehydrates from the server (contracts/provider-layer.md §4)
- [X] T014 [US1] Update `admin/src/pages/Settings.tsx`: provider `SingleSelect` sourced from `admin/src/data/providers.ts`; a curated `SingleSelect` when `MODELS[providerId]` exists and a plain text input when it does not; **remove the unguarded `MODELS[next][0].id` index**, which throws for a provider with no curated list; a Base URL input in its **own labelled field**, visibly separate from the credential and marked required for `requiresBaseUrl` providers — never a placeholder or hint on the key field; and an English note beside the model input when the provider has no curated list (contracts/provider-layer.md §5)
- [X] T014a [US1] Two pure-function suites, no Strapi runtime and no provider. **(a)** `supportsVision` from `server/src/services/providers.ts`: a table of `(descriptor id, model identifier) -> expected`, asserting each of the four declared rules of contracts/provider-layer.md §3 — every `true` case, every exclusion (`gpt-3.5`, the non-text families), and `openai-compatible` returning `false` for everything. This is the suite that makes finding U1 impossible to reintroduce: a descriptor silently reverted to bare default-deny turns the table red instead of passing every negative check. **(b)** `server/src/services/config.ts`: normalization keeps unknown provider keys, `isSet` is recomputed from the ciphertext and never trusted from input, a missing field takes its default, `baseUrl` validation accepts absolute `http:`/`https:` and rejects relative URLs and a userinfo component while trimming trailing slashes, and `isGroundingEnabled()` returns the AND of both switches across all four combinations (FR-006, FR-008, FR-036)
- [X] T015 [US1] Create `server/src/services/agent.ts` — build the per-request agent as `createAgent({ llm, tools, prompt, middleware: [modelCallLimitMiddleware({ runLimit: 8, exitBehavior: 'end' })] })` from `langchain` and run it as `agent.stream(state, { signal, recursionLimit })`. The middleware counts **model calls**, which is what today's `stopWhen: stepCountIs(8)` counted, so no super-step arithmetic is needed and the limit ends the turn cleanly instead of raising a `GraphRecursionError` the editor would see. `recursionLimit` stays only as a generous backstop against a loop that never reaches a model node. **Re-read `dist/agents/middleware/modelCallLimit.d.ts` after T002's install** — it was verified from the published 1.5.10 tarball, not from `node_modules`. Note the `'end'` path appends a synthetic English `AIMessage` naming the limit, which streams and is persisted (contracts/chat-stream.md §5). LangChain's `tool-approval-request` / `tool-approval-response` human-in-the-loop mechanism is **deliberately unused** — approval stays structural
- [X] T016 [US1] Register the new `agent` service in `server/src/services/index.ts`
- [X] T017 [US1] Rewrite `server/src/services/tools.ts` onto LangChain's `tool(fn, { name, description, schema })`, porting each tool **shape-for-shape**: the existing zod v4 schemas, the uid allow-list, the `permission-checker` check against the **caller's live** ability, the compact JSON result and the structured errors, all rebuilt per request (research D7, Principle II). The `mode` parameter and its gating stay for now — US2/T038 removes them, which is what keeps this story independently shippable
- [X] T018 [US1] Rewrite the request path in `server/src/controllers/chat.ts`: `toBaseMessages(replayed)` directly on the `UIMessage[]` the history rebuild already produces (**not** via `convertToModelMessages`), the agent stream from T015, then hand `pipeUIMessageStreamToResponse` a **`createUIMessageStream`** whose `execute` merges the bridge's chunks — `execute: ({ writer }) => writer.merge(toUIMessageStream(agentStream, { onError: log, onAbort }))`. **No `tee`.** Consume the agent through the LangGraph stream mode, not `streamEvents()`, or tool inputs never reach the UI (research D17). The route path, its policies, `useChat`, `DefaultChatTransport` and the wire protocol are **unchanged** — if the client needs a change, the contract is broken (contracts/chat-stream.md §1, §2, §6)
- [X] T019 [US1] Persistence in `server/src/controllers/chat.ts` is the SDK's `onFinish({ responseMessage })` on that same `createUIMessageStream` — the callback whose argument is field-for-field what the controller destructures today, so this task is now moving the existing body one call up rather than rebuilding a mechanism (research D5, corrected). **`originalMessages: messages` is required, not optional**: the bridge emits a bare `{ type: 'start' }` with no `messageId`, so without it the stored turn gets `id: ''` and the browser is sent no id at all. Keep the rest verbatim — a persistence failure logs through `redact.describeError` and **never** surfaces as a provider error (which now also covers anything thrown inside `execute`, since `createUIMessageStream` turns such a throw into an `{type:'error'}` chunk the editor sees); the user's turn is still persisted before streaming begins; file parts are filtered out so stored history never holds attachment bytes (contracts/chat-stream.md §3)
- [X] T020 [US1] Rewire stop in `server/src/controllers/chat.ts`: keep the Koa `close` / `aborted` distinction (a close before the stream finished is a real stop; after normal completion it is not), pass `abort.signal` into `agent.stream`, keep the partial output marked interrupted, leave the thread usable, and begin no further tool step after abort. Two verified traps: **`onFinish`'s `isAborted` is always `false` here** — it is set only by an `{type:'abort'}` chunk and the bridge never emits one, so the interrupted branch reads the Koa flag (or write the `abort` chunk yourself to make it honest); and **on an `AbortError` the bridge enqueues `{ type: 'error', errorText }`**, so a stop reads to the editor as a failure unless suppressed through the bridge's `onAbort`. Carry `data-interrupted` with `appliedSince({ threadId, ownerId, since })` by **writing it into the stream** — `writer.write({ type: 'data-interrupted', data })` after the merged stream drains — instead of splicing the assembled parts array, which drops the `as never` cast and makes the notice visible live rather than only on reload; catch `appliedSince`'s own failure locally (contracts/chat-stream.md §4)
- [X] T021 [US1] In `server/src/controllers/chat.ts`, drop image bytes from the outgoing message **before** `toBaseMessages` converts anything whenever `supportsVision` is `false`, while keeping the ordinal attachment manifest reaching the model as text so placement by filename still works (FR-006, FR-023)
- [X] T022 [US1] Error handling in `server/src/controllers/chat.ts`: all five `ProviderConfigError` codes answered as ordinary HTTP errors naming the provider **before** generation; provider errors during the stream logged through `redact.describeError` and surfaced generically unless `showProviderErrorDetails` is on, in which case the **redacted** detail is shown. Nothing credential-shaped is echoed on any path. **⚠ The SDK's `onError` mask does not guard this path.** The bridge catches a provider failure itself and enqueues `{ type: 'error', errorText: errorObj.message }` — the provider's raw message, straight to the browser, past every callback. Redaction must therefore be a **`TransformStream` over the merged chunk stream** that rewrites every `{type:'error'}` chunk's `errorText` through `redact.describeError` before it reaches the wire. All three hooks are used and none substitutes for another: the bridge's `onError(Error)` is the server-side logger, `createUIMessageStream`'s `onError(unknown) => string` maps anything that does escape, and the transform guards the path the provider's own text actually takes. This is an FR-008 / SC-009 hole, not a nicety (contracts/chat-stream.md §8)
- [X] T023 [US1] Write the US1 `README.md` delta: the shipped provider table, how a provider is configured, the base-URL field, and that a provider the adapter layer supports but the distribution does not carry is **absent from the selection rather than offered and broken** (FR-011, FR-053)
- [ ] T024 [US1] Verify quickstart scenarios A1-A14 in a running admin panel per `specs/003-langchain-content-assistant/quickstart.md` §A — one live send per shipped provider, provider switch under a minute with no restart, stop mid-turn, the direct-identifier round trip, a non-curated identifier still used verbatim, missing credential and missing base URL both refused before generation, an invalid base URL `400`, image **withheld** on a non-vision model (A10) **and image input still working on a vision-capable one (A13 — run the pair; a bare default-deny passes A10 and removes the capability silently)**, a redacted provider error, and the A14 credential sweep. A provider that has not answered a real message has not been verified  
      **STATUS: NOT DONE — requires a running Strapi admin panel**, which does not exist in this repository.
...
- [ ] T026 [US1] Run the per-commit gate for this phase: `corepack pnpm@10 run typecheck` clean, `corepack pnpm@10 run build` with `dist/` staged, and Principles I and II re-checked for the changes under `server/src/services/`  
      **STATUS: automated gate items done** (typecheck + test + build clean, `dist/` rebuilt, Principles I/II re-checked). Outstanding: the live admin-panel verification.
...
- [X] T027 [P] [US2] Delete `admin/src/components/ModeSelect.tsx`
- [X] T028 [P] [US2] Delete `admin/src/components/AuditReportCard.tsx`
- [X] T029 [P] [US2] Delete `server/src/services/audit-qa.ts` and `server/src/services/audit-security.ts`
- [X] T030 [P] [US2] Delete `server/src/policies/has-audit-permission.ts` (already unreferenced by any route — dead on arrival)
- [X] T031 [US2] Remove `mode`, `setMode`, `modeRef` and `changeMode` from `admin/src/hooks/useThreads.ts`
- [X] T032 [US2] Remove the mode wiring from `admin/src/pages/Chat.tsx`: the `mode` field leaves the chat transport body and the mode-conditional composer hint goes
- [X] T033 [P] [US2] Make the composer hint in `admin/src/components/Composer.tsx` always speak about approval, never about which mode is active (FR-017)
- [X] T034 [P] [US2] Remove the mode control from the header prop in `admin/src/components/ThreadSidebar.tsx`
- [X] T035 [US2] Delete the audit-report branch from `admin/src/components/MessageList.tsx`, so a stored `runQaScan` / `runSecurityAudit` part falls through to the generic tool pill — it reads as something that happened, **without implying the capability is still available** (contracts/removals.md §2)
- [X] T036 [US2] Stop resolving `mode` in `server/src/services/threads.ts`, and drop `setMode` and the `modeAtSend` passing
- [X] T037 [US2] Drop `mode` from the chat request schema in `server/src/controllers/chat.ts` and from its calls into `prompt.build` and `tools.buildTools`
...
- [X] T040 [US2] Remove from `server/src/types.ts`: `CHAT_MODES`, `ChatMode`, and `AuditKind`, `AuditSeverity`, `AuditCategory`, `AuditLocation`, `AuditFinding`, `AuditCoverage`, `AuditReport`, `AuditOptions` (data-model §8)
- [X] T041 [US2] Remove the `'audit-qa'` and `'audit-security'` service registrations from `server/src/services/index.ts` — a dead service that still registers is a remnant
- [X] T042 [US2] Remove the `'has-audit-permission'` policy registration from `server/src/policies/index.ts`
- [X] T043 [US2] Remove the `audit` config key from `server/src/config/index.ts` and `getAuditOptions()` from `server/src/services/config.ts`
- [X] T044 [US2] Unregister the `plugin::ai-content-studio.audit.run` permission action in `server/src/bootstrap.ts`. A role that was granted it must not break the upgrade — a stored grant for a no-longer-registered action is inert, so there is nothing to migrate
...
- [X] T070 [P] [US5] Create `admin/src/hooks/useCopy.ts` as a **thin wrapper over Strapi's own** `useClipboard()` from `@strapi/strapi/admin` — the same specifier this repo already imports `useNotification` and `useFetchClient` from (verified: `@strapi/strapi/dist/admin.d.ts` re-exports `@strapi/admin/strapi-admin`, which exports `useClipboard: () => { copy: (value: string | number) => Promise<boolean> }`, and it already guards empty and non-string input, returning `false` instead of throwing). What stays ours is **only** the insecure-context fallback: `useClipboard` has no `document.execCommand` path — confirmed absent from all of `@strapi/admin` and `@strapi/design-system` — and that fallback is the entire reason research D13 exists, because a Strapi panel served over plain HTTP on a LAN host has no `navigator.clipboard`. So: `copy()` → on `false`, hidden-textarea `execCommand` → on failure, an **explicit** message, never a silent no-op (FR-040)
- [X] T071 [US5] Create `admin/src/components/CopyButton.tsx` from `IconButton` (`@strapi/design-system` v2) plus the `Duplicate` icon (`@strapi/icons` v2) — **not** a hand-written `<button>`. `IconButton` requires `label`, renders a real `<button type="button">`, wraps the icon in `AccessibleIcon` (`aria-hidden` svg + visually-hidden label) and adds a tooltip by default, so FR-041's focusability, accessible name and pointer-free operation come from the component. The repo already ships this exact shape at [ThreadSidebar.tsx:196](admin/src/components/ThreadSidebar.tsx#L196). Two notes: the accessible name comes from a visually-hidden text node rather than an `aria-label` **attribute**, so assert on the accessible name (or pass `aria-label` explicitly — props spread through); and there is a competing local `styled.button` named `IconButton` at [styles.ts:102](admin/src/components/styles.ts#L102) used by `Composer.tsx` — pick the design-system one deliberately rather than by import order
- [X] T072 [US5] **Dropped.** T071 no longer hand-writes a button, so there are no copy-control styles to add to `admin/src/components/styles.ts`. This also dissolves the US5/US6 contention over that file recorded below — only T083 touches it now. Announce the copy outcome through what the panel already mounts: `useNotifyAT().notifyStatus` / `notifyAlert` write into the `role="status"` / `role="alert"` live regions that `DesignSystemProvider` renders app-wide, or `toggleNotification` (whose `Alert` carries `role="status"`, and `role="alert"` for `danger`) for the failure path FR-040 requires. **Call `useNotifyAT` from `MessageList` and pass the notifier down** — its cleanup clears all three regions when *any* consumer unmounts, so calling it inside a per-message `CopyButton` lets one unmount wipe another's pending announcement
- [X] T073 [US5] Wire the controls into `admin/src/components/MessageList.tsx`: one per assistant message copying that message's **Markdown source** (the `text` parts joined, which is the source as authored), one per code block copying **only** that block's contents without surrounding prose, **no control at all** on a message that is only a structured card unless a readable plain-text rendering of the card is offered — never a control that copies nothing — and either no control while a turn is still streaming or one that copies exactly what has arrived, never a partial value presented as complete (FR-038, FR-039, FR-043)
...
- [X] T076 [P] [US6] Add `publishRequested` and `publishConfirmed` (booleans) to `server/src/content-types/change-set/schema.json` (data-model §7)
...
- [X] T079 [US6] Derive the set status in `server/src/services/change-sets.ts`: `applied` **only** when every selected item's write reached `applied` **and** every publish outcome is `published` or `not_applicable`; anything else — a blocked publish included — is `partially_applied`, reported as partially applied and never as a success (FR-052). Any transition out of `pending` still revokes the set's previews
- [X] T080 [US6] Accept `publish` and `confirmPublish` in the apply body in `server/src/controllers/change-sets.ts` with zod validation, returning `409 publish_confirmation_required` for the mismatch and leaving every existing error code and status unchanged, so the risky action on an expired or already-resolved plan is refused with the **same** explanation as the existing approve actions
- [X] T081 [US6] Add `publish` and `confirmPublish` to the apply call in `admin/src/hooks/useChangeSet.ts`
...
- [X] T083 [US6] Add the risky-action styles to `admin/src/components/styles.ts` — now the **only** task touching that file, since T072 was dropped when T071 moved to the design system's `IconButton`. US5 and US6 no longer contend here
- [X] T084 [US6] Render the publish outcome per row in the apply report in `admin/src/components/MessageList.tsx`: what was written and, when the publish phase ran, whether the document was published, with the reason whenever either was refused. The report is appended to the conversation and persisted on the thread so a reload replays it rather than losing it to a toast (FR-050, US6-8)
- [X] T085 [US6] Write the US6 `README.md` delta: the new approval action, what its confirmation means, and that the existing approve actions are unchanged (FR-053)
...
- [ ] T093 Confirm all twelve success criteria SC-001..SC-012 listed in `specs/003-langchain-content-assistant/spec.md` against the observations recorded in T024, T048, T056, T068, T074 and T086, and note any that were not observed in a running admin panel rather than reported as met  
      **STATUS: NOT DONE — requires a running Strapi admin panel**, which does not exist in this repository.
- [X] T094 Verify quickstart G5-G6: `corepack pnpm@10 run typecheck` clean, and `dist/` staged with every source commit in this feature — no stale `dist/`
- [ ] T095 Walk `specs/003-langchain-content-assistant/quickstart.md` → Definition of done and confirm each line, including that the T001 constitution amendment landed **first** in its own `docs:` commit with an updated Sync Impact Report  
      **STATUS: NOT DONE — requires a running Strapi admin panel**, which does not exist in this repository.
- [X] T096 Re-read the composed instruction text end to end one final time against [contracts/instructions.md](contracts/instructions.md) §5, because a prohibition checked only by grep survives a paraphrase — no consuming project name, no hard-coded field names, no model identifier, no mode reference, English throughout
...
- **Phase 7 (US5)**: T070 (the hook) in parallel with T071 (the button) — different files, and T072 is dropped.
- **Phase 8 (US6)**: T076 in parallel with T077-T079, which are sequential in `change-sets.ts`.
- **Phase 9**: T089, T090 and T091 in parallel.
- **Across stories**: US5 and US6 can be worked concurrently with US3/US4 by a second pair of hands. Only **one** shared file remains — US5's T073 and US6's T084 both edit `admin/src/components/MessageList.tsx`. The `styles.ts` contention is gone with T072.
...
Each was found by checking the installed package rather than the plan's notes, which is what the
plan asked for in T002 and T015.

### 1. The bridge is `@ai-sdk/langchain@2.0.285`, not `3.0.93`

Research D17 flagged three risks in `3.0.93` and asked T002 to answer them against a real install.
All three are real, and all three are avoided by the 2.x line — which D17 itself lists as the
preferred fallback:

| D17 finding | `3.0.93` | `2.0.285` |
|---|---|---|
| ESM-only, no `require` condition | confirmed | **dual-format**, has a `require` condition |
| `engines: node >= 22` vs this repo's `>= 20` | confirmed | **`>= 18`** |
| hard `ai@7.0.93` against this repo's `ai@6` | confirmed — an *exact* pin, so it can never dedupe | **`ai@6.0.277`**, same major |

Choosing 2.0.285 avoids upgrading this repo to `ai@7`, which would have moved the wire and storage
format FR-013 exists to freeze. `toBaseMessages`, `toUIMessageStream` and its `onError` / `onAbort`
callbacks are unchanged in 2.0.285, and the full tool lifecycle is still emitted on the LangGraph
stream path.

To get exactly **one** copy of `ai`, `ai` and `@ai-sdk/react` are pinned to the versions the bridge
pins (`6.0.277` / `3.0.280`). `pnpm why ai` now shows one v6 copy shared by all three.
`@langchain/openai` is pinned to `1.5.5`, the newest release still declaring `node >= 20`; 1.5.10
moved to `>= 22`, which would have been a breaking change for consumers on Node 20.

### 2. `createAgent` takes `model` and `systemPrompt`, not `llm` and `prompt`

`plan.md` and research D1 specify `createAgent({ llm, tools, prompt })`. Neither option exists in
`langchain@1.5.10` as installed: `CreateAgentParams` declares `model: string |
AgentLanguageModelLike` and `systemPrompt?: string | SystemMessage`, and a grep for `llm?:` /
`prompt?:` across `dist/agents/*.d.ts` returns nothing.

The design intent is unaffected. `model` accepts an *instance*, and only the string branch resolves
through `initChatModel` — so D2's structural objection and Principle IV both still hold.

### 3. The bridge's `onAbort` cannot suppress its error chunk

`contracts/chat-stream.md` §4 suggested suppressing the abort-shaped error chunk "through the
bridge's own `onAbort`". It cannot: `onAbort` is `() => void`, and the bridge's `catch` block
enqueues `{ type: 'error', errorText }` **unconditionally**, after whichever callback ran.

So a stop would read to the editor as a failure. The fix is the `TransformStream` §8 already
requires for redaction: it drops the error chunk when the turn was stopped, and rewrites `errorText`
through `redact.describeError` otherwise. `writer.merge()` also returns `void` rather than a
promise, so the same transform's `flush()` is where `data-interrupted` is written — it is the one
place that reliably runs after the merged stream drains and before `onFinish` assembles the message.

### 4. `getPrivateAttributes` is not pure

T058 recommends Strapi's own schema predicates, and they are used. But
`contentTypes.getPrivateAttributes()` reads the **global `strapi`**
(`strapi?.config?.get('api.responses.privateAttributes')`), so it cannot run in a suite that is
forbidden from booting a host. It is therefore injected into the renderer at the service boundary,
where the global legitimately exists — keeping Strapi's predicate as the authority *and* the
renderer genuinely pure.

Separately, `getVisibleAttributes` does **not** exclude `createdBy` / `updatedBy`:
`getNonVisibleAttributes` never calls `getCreatorFields`. `grounding.test.ts` caught `createdBy`
rendering as a relation to `admin::user`; the creator fields are now subtracted explicitly.

### 5. A latent bug in thread condensation, introduced by the provider swap

`threads.summarizeTurns()` called the AI SDK's `generateText({ model })` with what had become a
LangChain `BaseChatModel`. It typechecked only because the service lookup is untyped, and the
failure would have been **invisible**: the surrounding `catch` degrades to the verbatim tail, so
condensation would simply never have worked again. It now calls the model instance's own `invoke()`.
Removing the `await import('ai')` also collapsed a code-split that had appeared in the server bundle.

### 6. Two small cleanups the feature's own goals implied

- `admin/src/data/models.ts` no longer exports `PROVIDERS` / `PROVIDER_LABELS`. Nothing read them
  once the provider catalog moved to `data/providers.ts` (D15), and they had become a second copy of
  the shipped-provider list. The `MODELS` literal is byte-identical and the session hook still
  parses all three groups.
- `services/config.ts` no longer seeds `activeModel` with a hardcoded model identifier — a second
  copy of an identifier CLAUDE.md says has exactly one home. A fresh install now reports
  `NO_ACTIVE_PROVIDER` until configured, which it could not have worked without anyway.
