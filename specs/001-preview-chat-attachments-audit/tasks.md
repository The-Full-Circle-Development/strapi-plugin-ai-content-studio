...
Strapi plugin, two halves plus a committed build (see plan.md → Project Structure):

- **Server**: `server/src/` — `content-types/`, `middlewares/`, `services/`, `controllers/`,
  `routes/`, `policies/`
- **Admin**: `admin/src/` — `pages/`, `components/`, `hooks/`, `translations/`
...
- [ ] T001 Extend the plugin config with `preview.{enabled:false,baseUrl,paths:{},ttlMinutes:30}`, `attachments.totalBudgetMb:50`, and `audit.timeBudgetSeconds:120` in server/src/config/index.ts
- [ ] T002 [P] Add shared feature types (`ChangeItem`, `ChangeSetStatus`, `AttachmentManifestEntry`, `AuditReport`, `AuditFinding`, `PreviewTokenPayload`) in server/src/types.ts
- [ ] T003 [P] Extract `redactSecrets` out of server/src/controllers/chat.ts into server/src/services/redact.ts, extend it to treat preview tokens as key-like, and re-point the chat stream error path at the shared helper
- [ ] T004 Add typed accessors `getPreviewOptions` / `getAttachmentOptions` / `getAuditOptions` in server/src/services/config.ts
...
- [ ] T005 [P] Create the `chat-thread` schema (hidden from Content Manager and CTB, `draftAndPublish:false`, `title`, indexed `ownerId`, `mode` enum, `lastActivityAt`, `contextSummary`, `summarizedThroughMessageId`) in server/src/content-types/chat-thread/schema.json
- [ ] T006 [P] Create the `chat-message` schema (`thread` relation, `role`, `sequence`, `parts` json, `attachmentManifest` json, `interrupted`, `modeAtSend`, `changeSet` relation) in server/src/content-types/chat-message/schema.json
- [ ] T007 [P] Create the `change-set` schema (`thread` relation, indexed `ownerId`, `status`, `items` json, `expiresAt`, `proposedAt`, `resolvedAt`, `approvedByUserId`, `approvedItemIds`, `destructiveConfirmed`) in server/src/content-types/change-set/schema.json
- [ ] T008 [P] Create the `preview-session` schema (`changeSet` relation, indexed `ownerId`, unique `sessionId`, `overlay` json, `stagedFiles` json, `expiresAt`, `revokedAt`, `targetUrl`) in server/src/content-types/preview-session/schema.json
- [ ] T009 Create server/src/content-types/index.ts exporting the four types and register `contentTypes` on the plugin in server/src/index.ts
- [ ] T010 [P] Add labelled-subkey derivation from `AI_STUDIO_ENC_KEY` plus `signPreviewToken` / `verifyPreviewToken` (HMAC-SHA256 over `{sessionId, ownerId, changeSetId, exp}`) in server/src/services/crypto.ts
- [ ] T011 [P] Register the `audit.run` permission action (section `plugins`, display name "Run AI Content Studio security audit") in server/src/bootstrap.ts
- [ ] T012 [P] Create the `has-audit-permission` policy and export it from server/src/policies/index.ts in server/src/policies/has-audit-permission.ts
- [ ] T013 Create the owner-scoped thread core — `createThread`, `getOwnedThread` (returns null for a foreign id so callers answer 404, super-admin included), `appendMessage` with monotonic per-thread `sequence`, `touchLastActivity` — and export it from server/src/services/index.ts, in server/src/services/threads.ts
- [ ] T014 Create `create` and `findOne` handlers (404, never 403, on a non-owned thread) and export them from server/src/controllers/index.ts, in server/src/controllers/threads.ts
- [ ] T015 Add `POST /threads` and `GET /threads/:id` under `admin::isAuthenticatedAdmin` in server/src/routes/index.ts
- [ ] T016 Gate `POST /chat` and every new admin route on the `chat.use` action via `admin::hasPermissions` in server/src/routes/index.ts
- [ ] T017 Require and zod-validate `threadId` on `POST /chat`, and persist the user turn and the assistant turn through `threads.appendMessage` from `onFinish`, in server/src/controllers/chat.ts
- [ ] T018 [P] Decompose admin/src/pages/Chat.tsx (814 lines) into a page shell plus admin/src/components/MessageList.tsx and admin/src/components/Composer.tsx with no behaviour change (R12)
- [ ] T019 Create the current-thread resolve/create hook and send `threadId` in the `DefaultChatTransport` body from admin/src/pages/Chat.tsx, in admin/src/hooks/useThreads.ts
...
## Phase 3: User Story 1 - Approve a change plan before anything is written (Priority: P1) 🎯 MVP
...
- [ ] T020 [US1] Implement `createPending` — validate `contentTypeUid` against the live `api::*` allow-list and the field path, RBAC-check the caller per item, read `currentValue`, compute `baseFingerprint` `{updatedAt, fieldHash}`, derive `destructive` and `resultingState`, persist `status:'pending'` with `expiresAt` — and export the service from server/src/services/index.ts, in server/src/services/change-sets.ts
- [ ] T021 [US1] Implement `apply` with the six-step gate in order (owned + `pending` + not expired → item exists and not `denied` → live per-item RBAC re-check → `baseFingerprint` re-check ⇒ `stale` → destructive confirmation → attachment resolutions present), then per-item outcomes and the `applied` / `partially_applied` transition, in server/src/services/change-sets.ts
...
- [ ] T023 [US1] Move `SYSTEM_PROMPT` out of server/src/controllers/chat.ts into server/src/services/prompt.ts and rewrite it around proposing — remove the write instructions, remove the hardcoded project field map and the "Concept Bath" identity, keep the permission-denied and Markdown rules
- [ ] T024 [US1] Remove the `createEntry`, `updateEntry`, and `publishEntry` tools in server/src/services/tools.ts
- [ ] T025 [US1] Add the `proposeChanges` tool — zod input, `blocked[]` for items the caller may not perform, `nextStep`, `empty_plan` on an empty items array, `unresolved_placement` with candidates rather than a guess — in server/src/services/tools.ts
...
- [ ] T030 [P] [US1] Create the plan card — per-item rows showing target type, document label, field, current → proposed value, resulting draft/published state, blocked reason — in admin/src/components/ChangePlanCard.tsx
...
- [ ] T034 [US1] Verify US1 in a real admin panel — quickstart scenarios 1 and 2, plus permission-denied paths 1, 2, and 4 from contracts/permissions.md

**Checkpoint**: US1 is fully functional and independently testable. This is the MVP — no content can be
...
- [ ] T035 [US2] Create the preview service — session create / lookup / revoke, precomputed `overlay` payload keyed by `contentTypeUid` + `documentId`, and the in-memory staged-file store bounded by `attachments.totalBudgetMb` — and export it from server/src/services/index.ts, in server/src/services/preview.ts
...
- [ ] T037 [US2] Implement the read-only overlay middleware — extract the token from `x-ai-studio-preview` or `?aiStudioPreview`, verify HMAC and `exp` before any database access, ignore an invalid token rather than erroring, load the session, require the change set still `pending`, `await next()`, walk `ctx.body.data` (array, object, `attributes`-shaped, and flattened v5 payloads) applying dotted-path overlays, rewrite attachment-fed media to a media-shaped object with a **negative `id`** and a staged-file URL, then set `Cache-Control: no-store` and `x-ai-studio-preview: applied` — in server/src/middlewares/preview-overlay.ts
...
- [ ] T043 [US2] Create the preview panel — Preview action that opens `previewUrl`, expiry messaging, and the in-panel field-level before/after comparison shown on `409` without blocking approval — in admin/src/components/PreviewPanel.tsx
- [ ] T044 [US2] Verify US2 in a real admin panel — quickstart scenario 3, plus permission-denied paths 9 and 10 from contracts/permissions.md
...
- [ ] T047 [US3] Add context condensing — send recent turns verbatim up to a token budget, replace older turns with a running `contextSummary` produced by the active provider, track `summarizedThroughMessageId`, refresh on crossing the threshold (R9, FR-021) — in server/src/services/threads.ts
- [ ] T048 [US3] Generate a short (≤ 60 char) automatic title from the first exchange, user-overridable, in server/src/services/threads.ts
...
- [ ] T055 [US3] Verify US3 in a real admin panel — quickstart scenario 4, plus permission-denied path 3 from contracts/permissions.md
...
- [ ] T060 [US4] Verify US4 in a real admin panel — scenario 5 of specs/001-preview-chat-attachments-audit/quickstart.md (output halts within ~2 s, no tool call in the server log after the press, interrupted marker survives a reload)
...
- [ ] T062 [US5] Build the tool set per `(caller ability, mode)` per contracts/model-tools.md — modes only ever narrow, never grant (FR-031) — in server/src/services/tools.ts
...
- [ ] T067 [US5] Verify US5 in a real admin panel — quickstart scenario 6, plus permission-denied path 7 from contracts/permissions.md
...
placement works on any provider, and enter the Media Library exactly once — only at apply time.
...
- [ ] T068 [US6] Create the attachments service — effective limits from the host's upload configuration, the per-conversation `totalBudgetMb`, and manifest validation — and export it from server/src/services/index.ts, in server/src/services/attachments.ts
...
- [ ] T073 [US6] Replace the media-id workflow in the system prompt with the ordinal workflow (`#1` → `attachmentOrdinal`, never a library id) in server/src/services/prompt.ts
...
- [ ] T075 [P] [US6] Create the attachments hook — browser-held `File` objects, 1-based ordinals stable for the conversation and never reused, pre-send validation against `GET /attachments/limits`, total-budget enforcement, ingestion-state tracking — in admin/src/hooks/useAttachments.ts
...
- [ ] T078 [US6] Verify US6 in a real admin panel — quickstart scenario 7, plus permission-denied path 5 from contracts/permissions.md
...
- [ ] T079 [US7] Create the QA service with the seven read-only checks from R7 — required field empty on an existing entry, relation pointing at a missing document, media field referencing a missing file, value outside an enumeration, component usage that cannot render, single type never created, published entry failing its own required fields — and export it from server/src/services/index.ts, in server/src/services/audit-qa.ts
...
- [ ] T082 [US7] Render the audit report — findings grouped by severity with counts, location, impact, remediation, and the coverage statement — in admin/src/components/MessageList.tsx
- [ ] T083 [US7] Verify US7 in a real admin panel — scenario 8 of specs/001-preview-chat-attachments-audit/quickstart.md (seeded defects reported, nothing modified, permission-limited coverage listed, clean type reports nothing, budget respected)
...
- [ ] T084 [US8] Create the security service covering the five areas of FR-046 — public/unauthenticated content-API exposure, create/update/delete/publish granted to the public role, roles holding permissions outside their stated scope, upload rules accepting executable or script types, `showProviderErrorDetails` and other debug settings, secret-like values in content fields or configuration — and export it from server/src/services/index.ts, in server/src/services/audit-security.ts
...
- [ ] T088 [US8] Verify US8 in a real admin panel — quickstart scenario 9, plus permission-denied path 6 from contracts/permissions.md, searching the whole report and the host server log for zero plaintext occurrences of the seeded fake key
...
- [ ] T096 Execute the full quickstart.md script (scenarios 1–10 including upgrade safety and provider switching) and all ten permission-denied paths, then complete the sign-off checklist in specs/001-preview-chat-attachments-audit/quickstart.md
...
- **US2 (Phase 4)**: depends on US1 (renders a pending change set) — genuine dependency, not optional
- **US3 (Phase 5)**: depends on Foundational only — can run in parallel with US1/US2
- **US4 (Phase 6)**: depends on Foundational (turn persistence, T017) — can run in parallel with US1–US3
...
Foundational ──┬──▶ US1 (P1) ──┬──▶ US2 (P2)
               │               ├──▶ US5 (P5) ──▶ US7 (P7) ──▶ US8 (P8)
               │               └──▶ US6 (P6) ─╌╌▶ (enriches US2 preview media)
               ├──▶ US3 (P3)
               └──▶ US4 (P4)
```
...
- The story's verification task last — it is the story's definition of done

### Parallel Opportunities

- **Setup**: T002 and T003 in parallel (T001 → T004 are sequential on config)
- **Foundational**: T005–T008 (four schema files) in parallel; then T010, T011, T012 in parallel with each other and with T018
- **US1**: T029 and T030 in parallel (different admin files) while the server chain T020 → T022 proceeds
- **US2**: T037 and T039 touch different files and can proceed once T035 lands
- **US3**: T051 in parallel with the server chain T045 → T048
- **US5**: T065 in parallel with T062 → T064
- **US6**: T075 in parallel with the server chain T068 → T071
...
2. Phase 2: Foundational (T005–T019) — **blocks everything**
3. Phase 3: User Story 1 (T020–T034)
4. **STOP and VALIDATE**: T034 — quickstart scenarios 1 and 2 plus denied paths 1, 2, 4
...
1. Setup + Foundational → conversations persist, owner-scoped
2. **+ US1** → nothing is written without approval (MVP, tag a release)
3. **+ US3** → the panel has a memory (highest daily-frustration win, independent of US1)
4. **+ US4** → Stop actually stops the server
5. **+ US2** → proposed values render on the real front-end
6. **+ US5** → modes narrow the tool set; unlocks the audits
7. **+ US6** → deferred ingestion; the Media Library stops collecting abandoned files
8. **+ US7** → QA pass
9. **+ US8** → security audit
10. Polish → README, expiry sweep, full quickstart, release gate
...
- Developer A: US1 → US2 (the change-set chain)
- Developer B: US3 → US4 (persistence and generation control)
- Developer C: US5 → US7 → US8 (modes and audits), starting once US1's `proposeChanges` lands
...
- **No test tasks by design** — Constitution V makes `pnpm run typecheck` plus the manual quickstart the
...
- `dist/` is rebuilt and staged with every source change — stale `dist/` is a shipped regression.
