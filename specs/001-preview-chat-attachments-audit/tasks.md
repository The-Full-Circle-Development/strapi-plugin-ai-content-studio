...
- [X] T013 Create the owner-scoped thread core — `createThread`, `getOwnedThread` (returns null for a foreign id so callers answer 404, super-admin included), `appendMessage` with monotonic per-thread `sequence`, `touchLastActivity` — and export it from server/src/services/index.ts, in server/src/services/threads.ts
...
- [X] T020 [US1] Implement `createPending` — validate `contentTypeUid` against the live `api::*` allow-list and the field path, RBAC-check the caller per item, read `currentValue`, compute `baseFingerprint` `{updatedAt, fieldHash}`, derive `destructive` and `resultingState`, persist `status:'pending'` with `expiresAt` — and export the service from server/src/services/index.ts, in server/src/services/change-sets.ts
- [X] T021 [US1] Implement `apply` with the six-step gate in order (owned + `pending` + not expired → item exists and not `denied` → live per-item RBAC re-check → `baseFingerprint` re-check ⇒ `stale` → destructive confirmation → attachment resolutions present), then per-item outcomes and the `applied` / `partially_applied` transition, in server/src/services/change-sets.ts
...
- [X] T023 [US1] Move `SYSTEM_PROMPT` out of server/src/controllers/chat.ts into server/src/services/prompt.ts and rewrite it around proposing — remove the write instructions, remove the hardcoded project field map and the "Concept Bath" identity, keep the permission-denied and Markdown rules
- [X] T024 [US1] Remove the `createEntry`, `updateEntry`, and `publishEntry` tools in server/src/services/tools.ts
- [X] T025 [US1] Add the `proposeChanges` tool — zod input, `blocked[]` for items the caller may not perform, `nextStep`, `empty_plan` on an empty items array, `unresolved_placement` with candidates rather than a guess — in server/src/services/tools.ts
...
- [X] T030 [P] [US1] Create the plan card — per-item rows showing target type, document label, field, current → proposed value, resulting draft/published state, blocked reason — in admin/src/components/ChangePlanCard.tsx
...
- [X] T035 [US2] Create the preview service — session create / lookup / revoke, precomputed `overlay` payload keyed by `contentTypeUid` + `documentId`, and the in-memory staged-file store bounded by `attachments.totalBudgetMb` — and export it from server/src/services/index.ts, in server/src/services/preview.ts
...
- [X] T043 [US2] Create the preview panel — Preview action that opens `previewUrl`, expiry messaging, and the in-panel field-level before/after comparison shown on `409` without blocking approval — in admin/src/components/PreviewPanel.tsx
...
- [X] T047 [US3] Add context condensing — send recent turns verbatim up to a token budget, replace older turns with a running `contextSummary` produced by the active provider, track `summarizedThroughMessageId`, refresh on crossing the threshold (R9, FR-021) — in server/src/services/threads.ts
- [X] T048 [US3] Generate a short (≤ 60 char) automatic title from the first exchange, user-overridable, in server/src/services/threads.ts
...
- [X] T062 [US5] Build the tool set per `(caller ability, mode)` per contracts/model-tools.md — modes only ever narrow, never grant (FR-031) — in server/src/services/tools.ts
...
- [X] T068 [US6] Create the attachments service — effective limits from the host's upload configuration, the per-conversation `totalBudgetMb`, and manifest validation — and export it from server/src/services/index.ts, in server/src/services/attachments.ts
...
- [X] T073 [US6] Replace the media-id workflow in the system prompt with the ordinal workflow (`#1` → `attachmentOrdinal`, never a library id) in server/src/services/prompt.ts
...
- [X] T075 [P] [US6] Create the attachments hook — browser-held `File` objects, 1-based ordinals stable for the conversation and never reused, pre-send validation against `GET /attachments/limits`, total-budget enforcement, ingestion-state tracking — in admin/src/hooks/useAttachments.ts
...
- [X] T079 [US7] Create the QA service with the seven read-only checks from R7 — required field empty on an existing entry, relation pointing at a missing document, media field referencing a missing file, value outside an enumeration, component usage that cannot render, single type never created, published entry failing its own required fields — and export it from server/src/services/index.ts, in server/src/services/audit-qa.ts
- [X] T080 [US7] Add the `audit.timeBudgetSeconds` deadline, the per-content-type sample cap (`maxEntriesPerType` default 50, max 200), and the mandatory `coverage` block listing `inspected`, `skippedForPermissions`, and `skippedForBudget` (FR-043, FR-044), in server/src/services/audit-qa.ts
- [X] T081 [US7] Add the `runQaScan` tool, built only in `audit` mode, with a description that forbids speculative findings so a clean project returns `findings: []` (FR-045), in server/src/services/tools.ts
- [X] T082 [US7] Render the audit report — findings grouped by severity with counts, location, impact, remediation, and the coverage statement — in admin/src/components/MessageList.tsx
...
- [X] T084 [US8] Create the security service covering the five areas of FR-046 — public/unauthenticated content-API exposure, create/update/delete/publish granted to the public role, roles holding permissions outside their stated scope, upload rules accepting executable or script types, `showProviderErrorDetails` and other debug settings, secret-like values in content fields or configuration — and export it from server/src/services/index.ts, in server/src/services/audit-security.ts
- [X] T085 [US8] Pass every `evidence` value through server/src/services/redact.ts **before the result leaves the tool**, so a secret appears only as a mask plus its location (FR-049, Constitution I), in server/src/services/audit-security.ts
- [X] T086 [US8] Add the `runSecurityAudit` tool gated on the caller's live `audit.run` ability, refusing with `{ ok:false, error:'permission_denied' }` and no counts, categories, or partial findings (FR-048), in server/src/services/tools.ts
- [X] T087 [US8] State in the audit prompt section that remediations are advice and that applying one goes through `proposeChanges` and the normal permission checks (FR-050) in server/src/services/prompt.ts
...
- [X] T089 [P] Document the new `audit.run` permission, the preview configuration and front-end contract, thread privacy including the deliberate lack of a super-admin exemption, and the accepted v1 limitations (no GraphQL overlay, single-instance staged media, held attachments lost on reload, audits never read project source files) in README.md
- [X] T090 [P] Add the new UI strings for the sidebar, mode selector, plan card, preview panel, and audit report in admin/src/translations/en.json
- [X] T091 Add the expiry sweep that resolves overdue pending change sets and revokes overdue preview sessions with their staged bytes in server/src/services/change-sets.ts and server/src/services/preview.ts
- [X] T092 Keep `sequence` consistent when the same user sends from two browser tabs of one thread so neither reply is lost or interleaved, in server/src/services/threads.ts
- [X] T093 Render history for a thread that references a removed content type and explain the reference is gone, in admin/src/components/MessageList.tsx
- [X] T094 Make every new failure an actionable message with no credential, raw provider text, or internal error (FR-053) across server/src/controllers/threads.ts, server/src/controllers/change-sets.ts, server/src/controllers/attachments.ts, and server/src/controllers/preview.ts
- [X] T095 Run `pnpm run typecheck` clean and `pnpm run build`, and stage the rebuilt dist/ with the source change (Constitution IV)
